...
const DIALOG_VARIANTS = [
  ...
  },
  {
    ...
    where: 'Ніде — екран, який його викликав, видалено разом із рештою успадкованого планування',
    open: 'Лише тут, у каталозі — значення лишається оголошеним навмисно: без імені наступний липкий пошук перепише pt-0 у власному className, а це вже дрейф',
  },
  {
    id: 'sheet',
  ...
  return (
    <ConfirmProvider>
      ...
      <PreviewBlock
        ...
        description="Точний shared organism з екрана клієнта: правий sm sheet, як форма створення клієнта. «Скасувати» позначена `dismiss` — вона нічого не робить, крім того, що робить × у шапці, тому нижче md футер її не малює: на телефоні футер — це стовпчик рядків на всю ширину, і дві однакові відповіді коштували б два з них. Позначають лише повтор: тут поряд стоїть «Зберегти», тож зникає саме зайвий рядок, а смуга футера лишається. Кнопку, яка у футері одна, не позначають — сама по собі вона читається не як повтор, а як дія, яку треба зробити, і нікому не заважає: такий футер на телефоні виглядає так само, як на десктопі. `dismiss` — це обіцянка, і `tests/dialog-dismiss-contract.test.mjs` перевіряє її: кнопка має бути у футері діалогу, шапка того діалогу має малювати ×, а обробник має бути тим самим onClose."
