...
const HELP_ARTICLE_CATALOGUE = Object.freeze([
  ...
  },
  {
    ...
    sections: [
      ...
      { id: 'queue', title: 'Одна черга для всієї підтримки', paragraphs: ['Розділ «Звернення» показує записи з усіх доступних проєктів, а не тільки призначені на вас. Можна перемикатись між дошкою і списком та фільтрувати за проєктом, чергою відповіді, відповідальним, пріоритетом, типом і періодом створення. Фільтр «Чекають на нас» / «Чекають на клієнта» відповідає на питання, чиє слово наступне, — статус про це не говорить. Фільтр за статусом є тільки у списку: на дошці статуси — це самі колонки, а які з них показувати, вирішує «Налаштування колонок».'] },
