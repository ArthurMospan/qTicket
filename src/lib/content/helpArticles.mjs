...
const HELP_ARTICLE_CATALOGUE = Object.freeze([
  ...
  },
  {
    ...
    sections: [
      { id: 'project', title: 'Один клієнт — один підготовлений простір', paragraphs: ['Власник або адміністратор відкриває «Клієнти» → «Новий клієнт», створює простір і додає внутрішніх працівників підтримки. Сторінка клієнта має три вкладки: «Звернення» з окремою чергою і показниками, «Люди» з командами клієнта та підтримки, «Налаштування» з контекстом і доступом. У вкладці «Люди» натисніть «Запросити клієнта», щоб надати першому представнику роль адміністратора саме цього простору. Кожне звернення отримує короткий номер на кшталт ACME-12, за яким його зручно називати в розмові.'] },
  ...
  },
  {
    ...
    relatedRoutes: ['/my?new=1', '/[projectId]'], minimumRole: 'client_member', relatedIds: ['issue-fields', 'statuses-and-categories'],
    sections: [
      { id: 'ways', title: 'Хто може створити звернення', paragraphs: ['Клієнт створює звернення у своєму єдиному клієнтському просторі та описує проблему, додає файли й подальші повідомлення. Внутрішній працівник може створити звернення з «Огляду», загальної черги або конкретного клієнтського простору й одразу обрати клієнта.'], bullets: ['Клієнту не пропонуються внутрішні поля керування: статус, пріоритет і відповідальний належать команді підтримки.', 'Після створення відкривається звернення з його спільною розмовою.', 'Поки email-відправку вимкнено, нові запрошення передаються клієнту скопійованою інструкцією через месенджер.'] },
  ...
  },
  {
    ...
    sections: [
      { id: 'profile', title: 'Профілі команди підтримки', paragraphs: ['Натисніть на учасника у «Команді», щоб побачити його роль, клієнтські простори й відкриті звернення. Клієнтські працівники керуються окремо у вкладці «Люди» свого клієнтського простору й не змішуються з внутрішньою командою.'] },
