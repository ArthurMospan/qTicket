...
const HELP_ARTICLE_CATALOGUE = Object.freeze([
  ...
  },
  {
    ...
    sections: [
      { id: 'ways', title: 'Хто може створити звернення', paragraphs: ['Звернення створює клієнт — у своєму проєкті: описує проблему, додає файли й подальші повідомлення. Підтримка звернення приймає, веде й закриває, але не відкриває його за клієнта: інакше десь у черзі з’явиться запис, про який уже не сказати, хто саме про нього просив.'], bullets: ['Кнопка «Створити звернення» стоїть у шапці «Огляду» — першого екрана після входу — і в самому проєкті. Обидві відкривають ту саму форму.', 'Ліворуч у клієнта три розділи: «Огляд» з показниками й останніми оновленнями, «Мої звернення» з усім списком і «Налаштування». Якщо клієнта запросили більш ніж до одного проєкту, замість «Мої звернення» зʼявляється «Проєкти» — список його проєктів, а кожен проєкт має власну чергу.', 'Клієнт бачить статус, тип і пріоритет свого звернення, але керують ними в підтримці: у клієнта ці поля показані, а не редаговані. Відповідальний працівник підтримки клієнту не показується взагалі.', 'Одне поле у зверненні належить клієнту: «Відповідальні клієнта» — хто з його власних співробітників веде це звернення. За замовчуванням це той, хто звернення створив; можна вибрати кількох. Підтримка це бачить і, за потреби, виправляє.', 'Після створення відкривається звернення з його спільною розмовою.', 'Поки email-відправку вимкнено, нові запрошення передаються клієнту скопійованою інструкцією через месенджер.'] },
