...
const HELP_ARTICLE_CATALOGUE = Object.freeze([
  ...
  },
  {
    ...
    sections: [
      ...
      { id: 'queue', title: 'Одна черга для всієї підтримки', paragraphs: ['Розділ «Звернення» показує записи з усіх доступних проєктів, а не тільки призначені на вас. Можна перемикатись між дошкою і списком та фільтрувати за клієнтом, статусом, відповідальним, пріоритетом, типом і періодом створення.'] },
  ...
  },
  {
    ...
    sections: [
      { id: 'ways', title: 'Хто може створити звернення', paragraphs: ['Звернення створює клієнт — у своєму єдиному проєкті: описує проблему, додає файли й подальші повідомлення. Підтримка звернення приймає, веде й закриває, але не відкриває його за клієнта: інакше десь у черзі з’явиться запис, про який уже не сказати, хто саме про нього просив.'], bullets: ['Кнопка «Створити звернення» стоїть у шапці «Огляду» — першого екрана після входу — і в самому проєкті. Обидві відкривають ту саму форму.', 'Ліворуч у клієнта три розділи: «Огляд» з показниками й останніми оновленнями, «Мої звернення» з усім списком і «Налаштування».', 'Клієнт бачить статус, тип і пріоритет свого звернення, але керують ними в підтримці: у клієнта ці поля показані, а не редаговані. Відповідальний працівник підтримки клієнту не показується взагалі.', 'Одне поле у зверненні належить клієнту: «Відповідальні клієнта» — хто з його власних співробітників веде це звернення. За замовчуванням це той, хто звернення створив; можна вибрати кількох. Підтримка це бачить і, за потреби, виправляє.', 'Після створення відкривається звернення з його спільною розмовою.', 'Поки email-відправку вимкнено, нові запрошення передаються клієнту скопійованою інструкцією через месенджер.'] },
  ...
  },
  {
    ...
    sections: [
      { id: 'search', title: 'Пошук', paragraphs: ['Для внутрішньої команди поле вгорі шукає звернення, проєкти й працівників підтримки. Клієнт шукає тільки звернення у своєму проєктрі. Точний номер звернення виходить першим, потім збіги в темі та описі. Показується лише те, до чого у вас є доступ.'] },
