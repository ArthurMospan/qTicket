...
// How a qTicket notification reads inside Telegram.
//
// Ported from QuickTeam, where this shape has been in front of people for a
// year, rather than invented again here. The event table is qTicket's: no chat,
// no calendar, no birthdays — and `incident_created`, the one event a support
// desk exists to react to.
//
...
export function formatTelegramNotification(items) {
  ...
  if (list.length === 1) {
    ...
    return {
      ...
      button: url ? { text: 'Відкрити в qTicket', url } : null,
  ...
  const heading = `🔔 <b>qTicket · ${list.length} ${plural(list.length, ['сповіщення', 'сповіщення', 'сповіщень'])}</b>`;
