...
- The engine is a task manager and stays one; the words a client reads are not. `issues`, `CreateTaskModal`, `TaskRow` and every other internal identifier are deliberate and must not be renamed — but nothing a `client_admin` or `client_member` can read may say «завдання», «задача», «спринт», «виконавець», «епік» or call their own support space a «проєкт». Their word for the record is «звернення» (their portal is «Мої звернення»); internal support's is «інцидент». Both halves live in `src/lib/content/incidentTerms.mjs` and a screen both audiences reach picks one by role rather than overwriting the other. Where the reader cannot be known when the copy is written — a notification title, a digest subject, an emailed comment, the shortcut sheet — the copy names the conversation or the incident key and not the record. `npm run test:unit` fails on the first task-manager word that returns to the client copy, the client portal, the client palette, the client tab title, the notification labels or the published help.
...
- Privileged creation/deletion of organizations, projects, issues, memberships, invitations, and API keys must go through authenticated server routes. Organizations and internal seats are created only by signed QuickTeam provisioning; `firestore.rules` refuses browser creation and update of the organization snapshot and every browser write to memberships.
