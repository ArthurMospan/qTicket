...
- The organization roles are `owner`, `admin`, `member`, `client_admin`, and `client_member`. `owner`, `admin`, and `member` are the tenant's internal support team; the two client roles are project-scoped external users. A `client_admin` may invite only `client_member` users into exactly one client project. Client roles may see that project's incidents, create incidents, and use the incident discussion, but may not change workflow state, priority, assignee, organization settings, or internal support chat.
- The engine is a task manager and stays one; the words anybody reads are not. `issues`, `CreateTaskModal`, `TaskRow` and every other internal identifier are deliberate and must not be renamed — but nothing a person can read may say «завдання», «задача», «спринт», «виконавець», «епік» or call a client's support space a «проєкт». The record is **«звернення»**, one word for both audiences: the client's portal is «Мої звернення» and the support queue is «Звернення», and `src/lib/content/incidentTerms.mjs` is the one table that spells it. Two vocabularies was the earlier answer and it was wrong — a customer's list and an agent's queue that are visibly not the same thing is a seam, and every shared screen had to remember which reader it was addressing. Where the reader cannot be known when the copy is written — a notification title, a digest subject, an emailed comment, the shortcut sheet — the copy names the conversation or the incident key and not the record. `npm run test:unit` fails on the first task-manager word that returns to the client copy, the client portal, the client palette, the client tab title, the notification labels or the published help.
