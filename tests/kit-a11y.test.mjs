...
test('the audit still sees what it is supposed to see', () => {
  ...
  // 3000 → 2800 → 2500 → 2250. Four deletions in one night — the third-party
  // integrations, then invoices, the timesheet and the analytics screens, then
  // the client's separate portal, then the workspace messenger and its own
  // component folder — took several hundred elements each. The floor still
  // catches a checker that stopped walking, which is what it is for; it is
  // lowered deliberately each time screens leave, never to make a red number go
  // away.
  assert.ok(committed.totals.elements > 2250, 'it reads every JSX element in them');
