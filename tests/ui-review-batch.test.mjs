...
test('a member profile offers qTicket actions only', async () => {
  ...
  // No action row at all: no circles, and nothing that opens a composer.
  assert.doesNotMatch(profile, /<IconAction/);
  assert.doesNotMatch(profile, /Створити інцидент/);
  assert.doesNotMatch(profile, /new=1/);
  // Writing to a colleague is not one of them either: a profile is a place to
  // look somebody up, and the product has one conversation — the incident's.
