...
test('панель пропонує одну дію і не пояснює, як вона влаштована', async () => {
  ...
  // Один компонент, не три: підпис групи ніде не малюється власними утилітами.
  ...
  // назва платформи не допомагає ніяк. Жоден рядок коду на цьому екрані не
  // називає нашу конфігурацію провайдера: «Способи входу» бачить лише клієнт,
  // а клієнт не має нашої консолі й не може оновити ні Client ID, ні Secret.
  // Коментар назвати Firebase може — це пояснення для того, хто читає код.
  const executable = page
    .split(/\r?\n/)
    .filter(line => !line.trimStart().startsWith('//'))
    .join('\n');
  assert.doesNotMatch(executable, /Firebase|Client ID|Client Secret/);
