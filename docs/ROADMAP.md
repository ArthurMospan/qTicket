...
- Organization roles are `owner`, `admin`, `member`, `client_admin`, and `client_member`. The first three belong to the tenant's support team. Client roles are restricted to their assigned project: they can see and create incidents and reply in an incident, while workflow, assignment, priority, and settings remain internal to **change**. Reading is a separate question and was conflated with changing until 2026-08-31: a client's board card had always drawn the status, the type badge and the priority mark, while the request's own page showed the status alone — so the page withheld from a customer what the card beside it had already told them. A client now reads status, type and priority on both surfaces and can set none of them. One fact stays withheld on both, deliberately: the resolution date, because a date a customer can read is a promised resolution time, and qTicket promises none. **That rule was prose only until 2026-09-02**: the composer obeyed it and the card, the list row and the request's attribute strip all drew the date for every reader, so the one fact the product withholds was on every surface a customer opens. The owner found it the same day the history was taught to withhold it — which had briefly made the product inconsistent the other way, visible everywhere and silent about changing. The code obeys the rule now, on all three, and `tests/qticket-roles.test.mjs` fails if any of them stops. **Which agent is answering is not withheld, and the rule that said it was is retired.** It was written on 2026-08-31 and the product never obeyed it: the request's attribute strip has shown the customer a read-only «Підтримка» cell, and the agent's name and face are on every reply in the shared conversation two centimetres to the right of it. A rule the code contradicts is worse than either answer, and hiding a cell whose content is legible beside it hides nothing. The owner settled it on 2026-09-01: the customer may see who is answering. What remains support's alone is *changing* it — the cell is read-only, because who takes a request is the desk's decision. A client admin may invite only client members, and only into a project they are on themselves. Letting them issue the second administrator's seat was built and withdrawn on 2026-09-02, on the owner's reasoning: granting a role is half a feature and the other half is taking it back, and qTicket has no screen that demotes a client administrator — deliberately, because the desk does not administer a customer's people and a customer's roster is not a place for a role editor. A one-way door is worse than a closed one. «Запросити співробітника» therefore draws the «Адміністратор» card **disabled**, pointing at the flow that does exist: support seats a `client_admin` from the client project's «Учасники» — or from the empty queue of a project no client has entered yet, which since 2026-09-02 carries the same button. The two invitations are one dialog in one shape, at the owner's request the same day: the desk sees the same two cards, both open, with the administrator's seat offered first; only who may pick which card differs. A client may be on **more than one project** as of 2026-09-01: `project.team` is an array and always was, so nothing in the data ever said otherwise — the screens said it by reading `projects[0]`, and the invitation route said it by refusing anybody who already held a seat. Inviting an existing client into a second project adds them to that project's team and **never moves their role**. Taking a seat away exists since 2026-09-03, and only as removal: the «Учасники» tab's kebab on a client row offers «Вилучити з проєкту» — the desk for anyone on the client's team, a `client_admin` for their own `client_member`s only, nobody for themselves — through `DELETE /api/projects/{id}/team/{uid}` (`remove:client_member`); when that was the person's last project in the organization the seat is archived under `orgMembershipArchive` and a new invitation restores it. Demotion is still not a thing.
...
- **The team and the brand are administered in QuickTeam's qTicket integration, not inside qTicket.** Staff seats, roles, identity, organization name, logo and colour arrive in the signed snapshot and are read-only here; a second editor for any of them is a copy that loses. The brand is one per tenant and every surface of qTicket wears it — the staff rail, the client portal, the organization picker and the tab title read the same `portalBranding`; the staff rail wore the organization's own name while the client's rail wore the desk name until 2026-09-02, and that was a defect, not a feature. What qTicket's own settings own is the support process and its record: statuses, types, priorities, labels and the archive. A settings section that does not administer one of those does not belong in qTicket.
...
| Client company visibility and administration | **Adapt** organization-level customer visibility | One qTicket client project is the customer's support space. `client_admin` invites only `client_member` into a project they hold themselves; no arbitrary cross-project sharing or request participants. |
