...
Ієрархія і зв’язки — це дві різні відповіді, і один прапорець колись давав на
них одну. Зв’язки між зверненнями підтримка бачить і створює: дублікат —
найчастіша операція служби підтримки, і донедавна не було чим його позначити.
Вони внутрішні, і межа проходить по ролі, а не по константі — `useIssueLinks`
отримує `internalViewer ? issueId : null`, тож клієнтська сесія не читає жодного
документа звʼязку, а секція, лічильник і «Додати зв’язок» стоять за
`!clientViewer`. Ієрархія лишається вимкненою за
`SHOW_INHERITED_TASK_HIERARCHY` у `src/components/workspace/IssueDetail.jsx`:
розбити звернення клієнта на дочірні означає, що клієнт дивиться на один запис,
поки робота живе в іншому. Дані, правила й інваріанти нижче стосуються обох.
...
екран бачить: `useNotifications`. Панель, що показує розмову звернення, публікує
її у стор — `visibleConversations` як стек, `visibleConversation` як його
верхівка (`{ kind: 'issue', id }`). Стек, а не одна комірка, бо звернення
відкривається вікном поверх іншого екрана, і після його закриття розмова під ним
й далі на екрані. `WorkspaceNotificationBridge` віддає хуку одну відповідь
...
непрочитаним, коли читач відкрив звернення, своє зробив — привів сюди — і
...
які продукує звернення; виняток — `emergency`, `alert` і `test`: екстрений
виклик має перебивати будь-що і не зникає мовчки. Вкладка у фоні — це «тебе там
немає»: такий запис лягає в дзвоник і чекає.
...
Самі сторінки записів у дзвонику не торкаються — ні звернення, ні бічна панель.
Раніше сторінка звернення тримала свій список типів, а сайдбар при вході в
проєкт гасив усі його записи разом і читав для цього всю колекцію сповіщень, тож
сповіщення про звернення, яких людина не відкривала, зникали, щойно вона зайшла
в сусіднє. Крапка біля проєкту в рейці гасне тоді, коли прочитано записи за
ним — у відкритому зверненні або в дзвонику.
